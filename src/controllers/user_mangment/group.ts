import { GROUPInfo } from '../../models/user_mangment/group'
...
export const show = async (req: Request, res: Response) => {
  try {
    ...
    const group = await info.show(req.body.id)
...
export const showByGroupName = async (req: Request, res: Response) => {
  try {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
      return res.status(200).json({ err: 'Invalid Data Passed!', errors: errors })
    ...
    const group = await info.showByName(req.body.name)
...
export const create = async (req: Request, res: Response) => {
  try {
    ...
    const newGroup = await info.create(req.body.group_name)
...
export const deleteGroup = async (req: Request, res: Response) => {
  try {
    ...
    const deleted = await info.deleteGroup(req.body.id)
    if (!deleted) {
      return res.status(404).json({
        status: 'error',
        message: 'can not find this id'
